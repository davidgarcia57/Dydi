...
<template>
  <Transition name="fade">
    <div v-if="showWakeup" class="fixed inset-0 z-[100] bg-cream flex flex-col items-center justify-center p-6 text-center">
      <div class="mb-8 relative">
        <div class="w-16 h-16 rounded-full border-4 border-hairline border-t-sage-deep animate-spin" />
        <div class="absolute inset-0 flex items-center justify-center">
          <span class="w-3 h-3 bg-terracotta rounded-full animate-pulse" />
        </div>
      </div>
      
      <h2 class="font-serif text-2xl font-semibold text-ink mb-3">
        Despertando los servidores...
      </h2>
      <p class="text-sm text-ink-soft max-w-xs mx-auto">
        Dado que usamos servidores gratuitos, pueden tardar hasta 40 segundos en despertar tras un periodo de inactividad.
      </p>
      <p class="text-xs font-bold text-terracotta mt-4">
