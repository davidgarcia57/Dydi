...
async function submit() {
  ...
  try {
    ...
    step.value = 'success'
    setTimeout(() => { showPlus.value = true }, 600)
  } catch (e) {
  ...
  <div class="min-h-screen bg-cream flex flex-col relative">
    ...
    <div v-else-if="step === 'no-habit'"
      ...
      <div class="w-20 h-20 rounded-full bg-amber-soft flex items-center justify-center mb-6">
        <svg class="w-10 h-10 text-amber-deep" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <button class="rounded-pill bg-ink text-paper px-8 py-3.5 font-bold text-sm"
    ...
    <div v-else-if="step === 'select'" class="flex-1 flex flex-col">
      ...
      <div class="px-6 pt-12 pb-6">
        <button
          class="w-9 h-9 rounded-full bg-surface border border-hairline
        ...
        <h1 class="font-serif text-3xl font-semibold text-ink leading-snug">
          ¿Cuál cumpliste?
        </h1>
      </div>
      ...
      <div class="px-6 space-y-3 pb-8">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            ...
            <p v-if="h.scheduled_time" class="text-xs text-ink-soft mt-0.5">{{ h.scheduled_time }}</p>
          </div>
    ...
    <div v-else-if="step === 'confirm'" class="flex-1 flex flex-col">
      ...
      <div class="flex items-center justify-between px-6 pt-12 mb-10">
        ...
        <!-- Racha chip -->
        <div class="flex items-center gap-1.5 rounded-pill bg-amber-soft
                    text-amber-deep px-3 py-1.5 text-xs font-bold">
          <span>★</span>
      ...
      <div class="px-8 mb-8">
        ...
        <div class="flex items-center gap-2 flex-wrap">
          <div class="flex items-center gap-1.5 rounded-full bg-surface border border-hairline
                      text-ink-soft px-3 py-1.5 text-xs font-semibold">
          ...
          <div v-if="selected?.scheduled_time"
      ...
      <div class="flex-1 flex flex-col items-center justify-center px-8">
        ...
        <button
          :disabled="submitting"
          class="w-24 h-24 rounded-full bg-sage-deep shadow-card flex items-center
                 justify-center mb-6 active:scale-95 transition-all duration-150
                 disabled:opacity-60 relative overflow-hidden"
          @click="submit"
        >
          <svg v-if="!submitting" class="w-12 h-12 text-paper" fill="none"
            viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div v-else-if="step === 'success'"
      ...
      <div class="relative mb-8">
        <!-- Outer glow ring -->
        <div class="w-32 h-32 rounded-full bg-sage-soft flex items-center justify-center
                    animate-[ping_0.6s_ease-out_1]" />
        <div class="absolute inset-0 w-32 h-32 rounded-full bg-sage-soft
                    flex items-center justify-center">
      ...
      <div class="relative rounded-card bg-paper shadow-card px-8 py-6 mb-10 w-full max-w-xs">
        <!-- +1 badge -->
        <div
          class="absolute -top-3 -right-2 rounded-full bg-terracotta text-paper
                 text-xs font-bold px-2.5 py-1 shadow-flat transition-all duration-500"
